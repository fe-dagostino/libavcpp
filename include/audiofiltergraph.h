...
#ifndef AUDIO_FILTERGRAPH_H
#define AUDIO_FILTERGRAPH_H
...
class CAudioFilterGraph : public IAVFilterGraph
{
public:
  /**
   * Empty constructor.
   */
  CAudioFilterGraph( const FString& sFilters );
  /**
   * Destructor method. 
   * Automatically release all resources, so if decoder status
   * is still open it will be closed during destroy.
   */
  virtual ~CAudioFilterGraph();
